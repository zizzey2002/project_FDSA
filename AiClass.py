...
class AiClient:
    def __init__(
        ...
        OPENAI_API_KEY="sk-dfb5d9295ee94adeafc438d18c7d4900",
    ...
    async def get_response(self, prompt: dict, only_response=True):
        response = await self.agent.ainvoke(prompt)
        if only_response:
            return response["messages"][1].content
        return response

    # def get_response(self, prompt: dict, only_response=True):
